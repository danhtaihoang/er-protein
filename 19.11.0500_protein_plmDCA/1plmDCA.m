...
function plmDCA_asymmetric(imputfile)
...
%Read inputfile (removing inserts), remove duplicate sequences, and calculate weights and B_eff.
    ...
    [N,B,q,Y] = load_alignments(inputfile);   % 2019.11.04: Tai
...
%denotes average.
    ...
    dlmwrite('di_plmDCA.dat',output,'precision',5)   % 2019.11.04: Tai
end
...
% 2019.11.04: Tai
function [N,B,q,Y]=load_alignments(inputfile)
    Y=importdata(inputfile);
    Y=Y+1;
    [B,N]=size(Y);
    q=max(Y(:));
end
