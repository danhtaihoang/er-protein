...
s = np.loadtxt('pfam_15selected.txt',dtype='str')
pfam_list = s[:,0]

#pfam_list = ['PF00060']
#pfam_list = ['PF00200']

ct_thres_list = [2.0,3.0,4.0,5.0]
#==========================================================================================
...
for pfam_id in pfam_list:
    try:
        ...
        #ct = np.loadtxt('../pfam_50_80pos/%s_ct.txt'%pfam_id).astype(float)
        di = np.loadtxt('%s/di.dat'%pfam_id).astype(float)    
        # find optimal threshold of distance
        ct_thres = np.linspace(2.0,10.0,17,endpoint=True)
        n = ct_thres.shape[0]
        ...
        print('auc max:',pfam_id,ct_thres[i0],auc[i0])
